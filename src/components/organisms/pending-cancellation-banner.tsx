...
import { cancellationReviewHref } from "@/lib/ui/inbox-grouping";

export type PendingCancellationBannerProps = {
  reason: string | null;
  requestedAt: Date;
  taskId: string | null;
  callId: string | null;
...
/** Striped red banner on the job page while a cancellation awaits office approval. */
export function PendingCancellationBanner({ reason, requestedAt, taskId, callId }: PendingCancellationBannerProps) {
  return (
    <div
      role="status"
    ...
    >
      ...
      <div className="flex items-center gap-3 text-sm font-medium">
        ...
        <Link href={cancellationReviewHref(taskId)} className="rounded-md bg-red-700 px-2.5 py-1 text-white hover:bg-red-800">
          Review in Inbox
        </Link>
      </div>
    </div>
