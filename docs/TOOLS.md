...
- 2026-09-02 v1.4 (W1-C) — Knowledge tools. Install-family detection uses an HVAC-install pattern (`system installation|new system|system relocation|air handler installation|zone system|<unit> install`) plus equipment material lines (brands, Heat Pump/Air Handler/Condenser/Fan Coil/Package unit) instead of a bare `/install/i`, which would also match the "Repairs & Part Installation" price-book prefix on ~700 repairs and plumbing installs. `get_visit_history.visits` are completed/in-progress jobs only; `one_line` is the *last substantive* tech note's first sentence (trailing "Followed up" / "Sent invoice" notes skipped), else the description. `get_open_balance` treats `jobs.outstanding_balance` as the truth (the invoices table lags it) and adds `invoice_status`, `address_label`, `customer_name`. `get_schedule` drops canceled jobs from `jobs`/`total` and reports them in `summary.canceled`; adds `summary.{installs, callbacks}`, `jobs[].{tags, is_install, is_callback, tech_ids}`, tech `gaps` bounded by `business_hours` (≥30 min between jobs, ≥60 min at the edges) with a spoken `label`; `window_label` fields across the unit are spoken style ("Wednesday September 2nd, 10 AM to noon"). `get_address_dossier` adds `street/unit/city`, `open_issue_details`, `open_balance_jobs`, `generated_at`, `source`; `open_balance_cents` is address-scoped and excludes deposits on not-yet-run jobs; access-note matching requires a door/gate/lockbox/keypad word (bare "garage" false-positives on "leak in the garage"); `speech_hint` flag priority is upcoming visit (≤14 days) → callback tag (≤12 months) → job waiting on us → open balance → labor warranty → recurring issue. `check_warranty` adds `address_label`, `installed_at`, `equipment[]` and echoes `equipment_hint`; `needs_office_confirmation` is true unless we installed the system *and* it is tagged `Registration Complete`. `get_job` adds `address_id`, `visit_date`.
- 2026-09-02 v1.5 (W1-B) — `find_availability`: a job blocks its tech from `scheduled_start` to `scheduled_end` (start + 2 h when null), capped at the end of the ET day it starts on (the import has 20 "scheduled" rows whose end is weeks after their start, 6 of them overlapping 09-02 → 09-15); a window is offered only when both the 2 h arrival window and the service duration end by `close`; slots are spread round-robin across open days, at most 2 per day unless a single day is requested (morning pick then afternoon pick); ranking is preferred tech → last tech at `address_id` → fewest blocking jobs that day → most lifetime jobs; extra result fields `range {from,to}` and `closed_days[]`; `limit` is optional (default 4, max 8, applied in the handler). `book_job` / `reschedule_job` return `outside_hours` for past or off-hours starts, `slot_taken` when the tech is busy, and also return `employee_id` (+ `new_window_start`/`new_window_end` on reschedule). `book_job` saves `caller_phone` with label `mobile` and no separate `customer.phone_added` event (one event per tool). `add_note` returns `{ note_id, job_id, seq }`. `request_cancellation` also accepts `in progress` jobs and stores `call_id` only when the call row exists. Events are emitted after the transaction commits (`emitEvent` is not transaction-aware); replays via `idempotency_key` emit nothing. Office helpers (`cancelJob`, `approveCancellation`, `rejectCancellation`, `assignJob`, `setJobStatus`) live in `src/domain/jobs.ts` / `change-requests.ts`; `rejectCancellation` opens the callback task inside its own single event.
