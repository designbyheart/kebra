...
| D5 | Auth on platform | **Decided: named logins.** Email + password, `users` table seeded from the office staff in the data (Alina, Audrey, Zoe, Ray, Logan, Andre as admins; Sonia as office; plus an owner account), bcrypt hashes, signed httpOnly session cookie, proxy guard. Actor on every event is the logged-in user. A grader account is included in the submission | — | Initial passwords (or I generate and put them in the submission notes) |
...
| **W1-F** | Auth: named office logins | 1 | W0 | fresh agent | 1.5h | Login page, session cookie, proxy, `users` seed from office staff, `currentUser()` helper used as event actor; agent webhooks authenticate by shared secret, not session |
...
- [x] Railway project `kebra-front-desk` (workspace "Design by Heart"; the personal workspace's trial had expired) with `kebra-web` + Postgres. Production URL: https://kebra-web-production.up.railway.app — health green 2026-09-02
- [x] Tavily API key (verified)
- [ ] (optional, stretch) Resend account + verified domain if we want to email callers a cancellation confirmation copy
- [ ] Your mobile number for handoff (D8) — reminder set; office passwords are generated by W1-F into `docs/CREDENTIALS.local.md` (gitignored)
- [ ] GitHub repo name (D10)
- [ ] A `receipts/` folder we fill as we go (Railway plan, Claude batch usage, Vapi minutes)
